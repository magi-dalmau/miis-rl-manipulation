...
def control_osc(dpose):
    ...
    u_null = u_null[:, :7]
    u_null = mm @ u_null
    u += (torch.eye(7, device=device).unsqueeze(0) -
          torch.transpose(j_eef, 1, 2) @ j_eef_inv) @ u_null
    return u.squeeze(-1)
...
assert controller in {
    "ik", "osc"}, f"Invalid controller specified -- options are (ik, osc). Got: {controller}"
...
asset_options.fix_base_link = True
# Part A
hole_part_a_asset = gym.create_box(
    sim, (external_size.x-hole_size.x)/2.0, hole_size.y, hole_asset_height, asset_options)
# Part B
hole_part_b_asset = gym.create_box(
...
part_a_1_pose.p = gymapi.Vec3(table_pose.p.x-(
    hole_size.x/2.0)-(external_size.x-hole_size.x)/4.0, table_pose.p.y, table_dims.z+0.5*hole_asset_height)
# part_a_1_pose.r = gymapi.Quat.from_axis_angle(
#     gymapi.Vec3(0, 0, 1), math.pi)
# Part A2
part_a_2_pose = gymapi.Transform()
part_a_2_pose.p = gymapi.Vec3(table_pose.p.x+(
    hole_size.x/2.0)+(external_size.x-hole_size.x)/4.0, table_pose.p.y, table_dims.z+0.5*hole_asset_height)
# part_a_2_pose.r = gymapi.Quat.from_axis_angle(
#     gymapi.Vec3(0, 0, 1), math.pi)
# Part B1
part_b_1_pose = gymapi.Transform()
part_b_1_pose.p = gymapi.Vec3(table_pose.p.x, table_pose.p.y-(
    hole_size.y/2.0)-(external_size.y-hole_size.y)/4.0, table_dims.z+0.5*hole_asset_height)
# part_b_1_pose.r = gymapi.Quat.from_axis_angle(
#     gymapi.Vec3(0, 0, 1), math.pi)
# Part B2
part_b_2_pose = gymapi.Transform()
part_b_2_pose.p = gymapi.Vec3(table_pose.p.x, table_pose.p.y+(
    hole_size.y/2.0)+(external_size.y-hole_size.y)/4.0, table_dims.z+0.5*hole_asset_height)
...
for i in range(num_envs):
    ...
    # add hole
    part_a_1_handle = gym.create_actor(
        env, hole_part_a_asset, part_a_1_pose, "hole_part_a_1", i, 0)
    part_a_2_handle = gym.create_actor(
        env, hole_part_a_asset, part_a_2_pose, "hole_part_a_2", i, 0)
    part_b_1_handle = gym.create_actor(
        env, hole_part_b_asset, part_b_1_pose, "hole_part_b_1", i, 0)
    part_b_2_handle = gym.create_actor(
        env, hole_part_b_asset, part_b_2_pose, "hole_part_b_2", i, 0)

    # add box
    box_pose.p.x = table_pose.p.x + np.random.uniform(-0.2, 0.1)
    box_pose.p.y = table_pose.p.y + np.random.uniform(-0.3, 0.3)
    box_pose.p.z = table_dims.z + 0.5 * box_height
    ...
    box_idx = gym.get_actor_rigid_body_index(
...
down_q = torch.stack(
    num_envs * [torch.tensor([1.0, 0.0, 0.0, 0.0])]).to(device).view((num_envs, 4))
...
while not gym.query_viewer_has_closed(viewer):
    ...
    yaw_dot = torch.bmm(box_yaw_dir.view(num_envs, 1, 3),
                        hand_yaw_dir.view(num_envs, 3, 1)).squeeze(-1)
    ...
    above_box = ((box_dot >= 0.99) & (yaw_dot >= 0.95) &
                 (box_dist < grasp_offset * 3)).squeeze(-1)
    ...
    goal_rot = torch.where(return_to_start, init_rot,
                           quat_mul(down_q, quat_conjugate(yaw_q)))
    ...
    grip_acts = torch.where(close_gripper, torch.Tensor(
        [[0., 0.]] * num_envs).to(device), torch.Tensor([[0.04, 0.04]] * num_envs).to(device))
    pos_action[:, 7:9] = grip_acts

    # Deploy actions
    gym.set_dof_position_target_tensor(sim, gymtorch.unwrap_tensor(pos_action))
    gym.set_dof_actuation_force_tensor(
        sim, gymtorch.unwrap_tensor(effort_action))
